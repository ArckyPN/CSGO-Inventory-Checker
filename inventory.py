...
class Inventory:
	...
	def saveBaseInv(self):
		...
		f.write("{name:{nameLen}};{num:>{numLen}};{price:>{priceLen}}\n".format(name="Name",nameLen=self.nameLength, num="Num",price="Price",numLen=self.numLength,priceLen=self.priceLength))
		for s in self.baseInv:
			f.write("{name:<{nameLen}};{num:>{numLen}};{price:>{priceLen}.2f}€\n".format(name=s["name"],nameLen=self.nameLength,num=s["num"],numLen=self.numLength,price=s["price"],priceLen=self.priceLength-1))
	...
	def saveRawInventory(self):
		...
		if glob.glob("inventory.txt"):
			preData = np.genfromtxt("inventory.txt", delimiter=';', skip_header=1, skip_footer=6, dtype=None, encoding=None, autostrip=True, usecols = (0,1,2))
			first = True
		...
		f.write("{name:{nameLen}};{num:>{numLen}};{price:>{priceLen}};{total:>{priceLen}} | {profit:>{priceLen}};{totalP:>{priceLen}}\n".format(name="Name",nameLen=self.nameLength, num="Num",price="Price",total="Total",profit="Profit",totalP="Total",numLen=self.numLength,priceLen=self.priceLength))
		totalProfit = 0.0
		totalItems = 0
		for s in self.stock:
			new = True
			if first:
				for d in preData:
					if s["name"] == d[0]:
							...
							f.write("{:{}};{:{numLen}};{:{priceLen}.2f}€;{:{priceLen}.2f}€ | {:>+{priceLen}}€;{:>+{priceLen}}€\n".format(s["name"],self.nameLength,  s["num"], s["price"], np.round(s["num"] * s["price"], 2), profit, np.round(profit * s["num"], 2),numLen=self.numLength,priceLen=self.priceLength-1))
							totalProfit += np.round(s["num"] * profit, 2)
							totalItems += s["num"]
							new = False
							break
			if new:
				f.write("{:{}};{:5};{:6.2f}€;{:7.2f}€ | new Item!\n".format(s["name"],self.nameLength,  s["num"], s["price"], np.round(s["num"] * s["price"], 2)))
				totalItems += s["num"]
